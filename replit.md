...
- **Modules**:
    ...
    - **Notifications**: Real-time system notifications with unread counts and historical views. Powered by `NotificationService` — notifications are stored in PostgreSQL and served via real API endpoints (not stubs). Notification triggers wired for: task assignment, expense submission (notifies admins/managers), invoice paid (notifies finance/management), quotation accepted (notifies sales/management). All notification routes protected by `requireAuth` middleware — no hardcoded dev user IDs. Bell badge clears immediately when user opens a conversation.
    - **Messaging**: Internal direct-messaging system. Users can start DM conversations with colleagues, send/receive messages in real time (5s polling). Unread badge shown in sidebar (green, 15s polling). Sending a message triggers an in-app notification and SMTP email ONLY for the first unread message — subsequent messages in an active unread thread do NOT create duplicate notifications or send emails (anti-spam logic). When user opens/reads a conversation, all DM notifications are cleared. Tables: `conversations`, `conversation_participants`, `messages`. Routes in `server/messaging-routes.ts`. Page at `client/src/pages/Messages.tsx`.
    - **Deferred Email Strategy** (`server/messaging-email.ts`): In-app notifications are immediate. Emails are deferred by 3 minutes using in-memory setTimeout. If multiple messages arrive in the same conversation within the delay, the timer resets and previews are batched into one summary email. If the recipient reads the conversation before the timer fires, the email is cancelled entirely. `notificationService` skips email for `direct_message` type — `messaging-email.ts` owns that channel.
